...
| `server/lib/task-progress.js` | Bounded, stat-cached reducer for nullable task progress on session APIs. It normalizes Claude Task*/TodoWrite/lifecycle observations, Codex `update_plan` (including executable unified-`exec` wrappers), and subagent transcripts into owner-attributed snapshots. Real top-level Claude human turns and Codex `task_started` records reset every prior owner; new subagent turns reset that owner only; harness task notifications do not reset state. Claude turn-end records plus Codex `task_complete` / `turn_aborted` discard owner snapshots with unfinished work but retain fully completed/cancelled history. Persisted Claude prompt/terminal events duplicate these boundaries so websocket refetches stay correct when a hook arrives before its transcript marker. A latest work item with no fresh tracker therefore returns `null`, preventing historical in-progress lists from surviving into unrelated or finished work. **Incremental cache:** first contact parses at most the newest 32 MiB; later growth on the same inode resumes at the byte after the last complete JSONL line, retaining pending tool calls and observations from the newest 32 MiB of the file (exactly what a fresh tail parse would see) across increments. Truncation, rotation, or growth beyond 32 MiB since the last read resets to a fresh tail parse. `DASHBOARD_TASK_SUMMARY_TTL_MS` (default `2000`, `0` parses every append immediately) additionally reuses a just-parsed display-only result inside a burst; `todo_summary` / `todo_snapshot` are attached to responses in `routes/sessions.js` and feed no server-side logic. First-line timestamps used for subagent owner mapping are cached per path (append-only files can never change line one), with a cheap `statSync` so a truncated or replaced file drops its entry. |
| `server/lib/event-payload.js` | Pure `trimHookPayload()` applied to a hook payload right before it is stored as `events.data` (and by the history importer's subagent merge): drops the native tools' whole-file mirrors, cuts strings to `DASHBOARD_EVENT_STRING_CAP`, keeps only the short scalars of a field over `DASHBOARD_EVENT_FIELD_CAP`, and records every change under `data._trimmed`. The in-memory payload the route keeps using is never touched. `scripts/trim-events.js` applies the same rules to rows stored earlier. |
...
| `lib/session-liveness.js` | Provider-aware process-liveness probes for the watchdog's dead-session reap. `probeAgentPidLive(pid, startToken, binary)` is the exact per-session check: signal-0 existence test, args match, then a start-of-process token comparison (`/proc/<pid>/stat` starttime on Linux, `ps -o lstart=` elsewhere) that defeats PID reuse — a missing token yields no answer rather than a verdict. `findAgentAncestor(binary, fromPid)` is the sender side: walks the process ancestry (per-hop `/proc` reads on Linux; ONE bounded `ps -Ao pid=,ppid=,args=` snapshot walked in memory elsewhere, keeping the hook path fast) to the owning CLI and returns `{ pid, start }` — used by `scripts/hook-handler.js` to stamp every payload. `probeLiveCwds(binary)` is the legacy fallback: enumerates matching `claude` or `codex` CLI processes (`ps -Ao pid=,args=`, then `lsof -a -p <pids> -d cwd -Fn` on macOS or `/proc/<pid>/cwd` on Linux) and returns the set of their working directories; `isAgentCommand()` matches the requested bare binary and `node`/`bun`-launched shims while rejecting lookalikes (`claude-mem`, `Claude.app`). Fail-safe by contract: probes return `available: false` (callers must change nothing) on Windows, inside containers (reuses `isInsideContainer` from `scripts/install-hooks.js` — host processes are invisible there, so an empty list would lie), on `ps`/`lsof` failure, or when `DASHBOARD_LIVENESS_PROBE=0` |
...
| `server/lib/compact-manager.js` | Reads the local **compact-manager** CLI's structured overview (`overview --json`: context auto-compaction mode, thresholds, per-model overrides, watcher health with attention flags, per-session usage keyed by Claude Code session UUID). One bounded `cross-spawn` per call with a hard timeout and output caps; a missing, failing, hung, or garbage-printing CLI resolves to `{ available: false, reason }` — never throws. Binary resolved from `DASHBOARD_COMPACT_MANAGER_BIN`, defaulting to PATH lookup |
| `server/routes/compact-manager.js` | `GET /api/compact-manager/status` — on-demand fetch of the compact-manager overview (no background poller; the Dashboard panel polls). Provider-injectable via `app.locals.compactManagerProvider` for tests. `available: false` is still HTTP 200 so clients can quietly hide the panel on machines without the CLI |
