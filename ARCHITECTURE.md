...
| `server/lib/event-payload.js` | Pure `trimHookPayload()` applied to a hook payload right before it is stored as `events.data` (and by the history importer's subagent merge): drops the native tools' whole-file mirrors plus the top-level `background_tasks` field from every hook (recording its JSON byte size at `data._trimmed.dropped.background_tasks`), cuts every string anywhere in the payload to `DASHBOARD_EVENT_STRING_CAP`, and applies the whole-field `DASHBOARD_EVENT_FIELD_CAP` only to `tool_input` / `tool_response`, keeping their short scalars when over budget. Every change is recorded under `data._trimmed`; `DASHBOARD_EVENT_STRING_CAP=0` disables all trimming. The in-memory payload the route keeps using is never touched. `scripts/trim-events.js` applies the same rules to rows stored earlier. |
