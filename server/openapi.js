  ...
  const spec = {
    ...
    tags: [
      ...
      },
      {
        ...
        description:
          "Readout of the local compact-manager CLI: context auto-compaction mode, thresholds, watcher health, and per-session usage. Absent CLI degrades to available:false.",
      },
      {
        name: "Remote Sources",
        description:
        ...
        HookEventRequest: {
          ...
          properties: {
            ...
            sender: {
              ...
              nullable: true,
              description:
                "Owner-process report attached by the local hook handler: either { pid, start } identifying the provider CLI process that owns the session (start = PID-reuse-guard start-of-process token), or { lookupFailed: true } when the ancestry walk could not identify it (which clears any recorded owner). Honored only on direct-loopback requests; other API clients should omit this field entirely (an absent sender makes no ownership claim).",
              properties: {
                pid: { type: "integer" },
                start: { type: "string" },
                lookupFailed: { type: "boolean" },
              },
    ...
    paths: {
      ...
      "/api/compact-manager/status": {
        get: {
          tags: ["Compact Manager"],
          summary: "Readout of the local compact-manager CLI (context auto-compaction)",
          operationId: "getCompactManagerStatus",
          responses: {
            200: {
              description:
                "Snapshot envelope. available:false (with a reason string) when the CLI is not installed or fails — clients should hide their panel then. When available, `overview` carries the CLI's `overview --json` output: mode, thresholds, per-model overrides, watcher rows with attention flags, and per-session usage keyed by Claude Code session UUID.",
              content: {
                "application/json": {
                  schema: {
                    type: "object",
                    additionalProperties: true,
                    description:
                      "Fields: available (boolean), fetched_at (epoch ms), reason? (string), overview? (object).",
                  },
                },
              },
            },
