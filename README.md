...
| **Dashboard**                      | Two tabs persisted in `localStorage`: **Monitor** — overview stats (6 stat cards, with the cost card labeled **API-Equiv. Cost** — totals are computed at provider API list prices (Anthropic rate cards for Claude sessions, OpenAI rate cards for Codex), not subscription spend), active agent cards with collapsible subagent hierarchy and a **context-fullness gauge** (latest request's context occupancy vs the model's window, green/amber/red at 70%/90%), and recent activity feed where consecutive `PreToolUse`/`PostToolUse` runs collapse into expandable "*N* tool calls · Read ×3, Bash ×2" rows (prompts, stops, errors, and notifications stay top-level), with dynamic item counts that fill available viewport height via `ResizeObserver`. **Health** — composite system health score ring (weighted: 0.4 × success rate + 0.25 × cache hit rate + 0.25 × (100 − error rate) + 0.1 × (100 − heap %)), storage engine donut chart with record distribution, cache performance / error rate / success rate gauges plus a usage-limit **Limits** pill (`rate_limit` throttling notices are reported there and excluded from the error rate — being rate-limited is not a failure), tool invocation horizontal bar chart (top 8), subagent effectiveness bars, model token distribution, and compaction impact stats. All health metrics auto-refresh every 30 s from `/api/settings/info` and `/api/workflows`. Cursor-following tooltips with viewport edge detection on every chart |
...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Hook payloads are **trimmed before they are stored** as `events.data`: the native tools' whole-file mirrors (`originalFile` on Edit/Write, `file.content` / `file.base64` on Read) are dropped; the top-level `background_tasks` field is dropped from every hook, with its JSON byte size recorded at `data._trimmed.dropped.background_tasks`; and every string anywhere in the payload is cut to this many characters (plus a short note of how much was cut). Every change is noted under `data._trimmed` (shown as **Trimmed on storage** in the event detail pane). With trimming on, what is stored is a preview: the terminal view and the diff of any edit under the field cap keep working, the "original file" pane is not shown for stored events, summaries count preview lines, and text search over events sees only what was kept — the transcript on disk keeps the full text. Without this, PostToolUse rows grow the database by hundreds of MB a week. Set to `0` to disable all trimming, including field drops |
| `DASHBOARD_EVENT_FIELD_CAP` | `16384` | Byte budget for a `tool_input` / `tool_response` that is still large after string trimming (a `structuredPatch` covering a whole file, say): over budget, only the short scalars that fit are kept. `npm run trim-events` applies the same rules to rows stored before trimming existed or restored from an older export (read-only dry run by default; `--yes --backup` to rewrite and VACUUM with the dashboard stopped) |
| `DASHBOARD_COMPACT_MANAGER_BIN` | `compact-manager` (PATH lookup) | Executable for the optional **compact-manager readout** panel on the Dashboard (context auto-compaction: mode, thresholds, watcher health, per-session usage via `GET /api/compact-manager/status`). The panel appears only when the CLI responds — a machine without compact-manager just never shows it, no configuration needed |
...
| `npm run trim-events` | Shrink `events.data` rows stored before payload trimming existed (dry run by default; `--yes --backup` to rewrite and VACUUM with the dashboard stopped) |
