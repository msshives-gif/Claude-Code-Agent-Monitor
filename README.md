...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Hook payloads are **trimmed before they are stored** as `events.data`: whole-file mirrors (`originalFile` on Edit/Write, `file.content` / `file.base64` on Read) are dropped, every string inside `tool_input` / `tool_response` is cut to this many characters, and each change is noted under `data._trimmed`. The transcript on disk keeps the full text; the event detail pane shows the preview. Without this, PostToolUse rows grow the database by hundreds of MB a week. Set to `0` to store payloads untouched |
| `DASHBOARD_EVENT_FIELD_CAP` | `16384` | Byte budget for a `tool_input` / `tool_response` that is still large after string trimming (a `structuredPatch` covering a whole file, say): over budget, only its short scalars are kept. `npm run trim-events` applies the same rules to rows stored before trimming existed (dry run by default; `--yes --backup` to rewrite and VACUUM with the dashboard stopped) |
