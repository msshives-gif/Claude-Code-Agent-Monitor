...
| `DASHBOARD_TASK_SUMMARY_TTL_MS` | `2000` | Serve-stale window (ms) for the per-transcript task-progress cache behind `include_task_progress` list requests. A transcript being actively appended to can never hit the size+mtime cache key, so without this floor a burst of list reloads (e.g. the dashboard refreshing on hook-driven WebSocket events) re-parses a multi-MB live transcript once per request. Within the window a just-parsed (slightly stale) summary is returned instead; set to `0` to restore immediate re-parse on every change |
