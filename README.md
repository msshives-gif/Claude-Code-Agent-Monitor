...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Hook payloads are **trimmed before they are stored** as `events.data`: the native tools' whole-file mirrors (`originalFile` on Edit/Write, `file.content` / `file.base64` on Read) are dropped, every string inside `tool_input` / `tool_response` is cut to this many characters, and each change is noted under `data._trimmed` (shown as **Trimmed on storage** in the event detail pane). What is stored is a preview: the diff and terminal views keep working, the "original file" pane is not shown for stored events, summaries count preview lines, and text search over events sees only what was kept — the transcript on disk keeps the full text. Without this, PostToolUse rows grow the database by hundreds of MB a week. Set to `0` to store payloads untouched |
