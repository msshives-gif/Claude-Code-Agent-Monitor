...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Payload của hook được **cắt bớt trước khi lưu** vào `events.data`: bản sao toàn bộ tệp của các công cụ gốc (`originalFile` với Edit/Write, `file.content` / `file.base64` với Read) bị loại bỏ, mọi chuỗi trong `tool_input` / `tool_response` bị cắt còn chừng này ký tự, và mỗi thay đổi được ghi vào `data._trimmed` (hiển thị là **Đã cắt bớt khi lưu** trong chi tiết sự kiện). Thứ được lưu là bản xem trước: khung terminal và diff của các chỉnh sửa dưới giới hạn trường vẫn hoạt động, khung "original file" không hiển thị cho sự kiện đã lưu, phần tóm tắt đếm số dòng của bản xem trước, và tìm kiếm văn bản chỉ thấy phần được giữ lại — transcript trên đĩa vẫn giữ toàn bộ văn bản. Đặt `0` để lưu payload nguyên vẹn |
