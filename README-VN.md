...
| **Bảng điều khiển**                      | Hai tab lưu trong `localStorage`: **Monitor** — số liệu thống kê tổng quan (6 thẻ, trong đó thẻ chi phí được ghi nhãn **API-Equiv. Cost** — tổng được tính theo giá niêm yết API của nhà cung cấp (biểu giá Anthropic cho phiên Claude, biểu giá OpenAI cho Codex), không phải chi tiêu thuê bao), thẻ Agent đang hoạt động với hệ thống phân cấp Subagent có thể thu gọn và **thước đo độ đầy ngữ cảnh** (mức chiếm dụng ngữ cảnh của yêu cầu mới nhất so với cửa sổ của mô hình, xanh/hổ phách/đỏ tại 70%/90%), nguồn cấp dữ liệu hoạt động gần đây nơi các chuỗi `PreToolUse`/`PostToolUse` liên tiếp được gộp thành hàng có thể mở rộng "*N* tool calls · Read ×3, Bash ×2" (prompt, stop, lỗi và thông báo vẫn ở cấp cao nhất), với số mục hiển thị tự động lấp đầy chiều cao viewport qua `ResizeObserver`. **Health** — vòng điểm sức khỏe hệ thống tổng hợp (trọng số: 0,4 × tỷ lệ thành công + 0,25 × tỷ lệ cache hit + 0,25 × (100 − tỷ lệ lỗi) + 0,1 × (100 − heap %)), biểu đồ donut phân bổ bản ghi, thước đo hiệu suất cache / tỷ lệ lỗi / tỷ lệ thành công cùng ô **Limits** cho giới hạn sử dụng (thông báo `rate_limit` được báo cáo ở đó và loại khỏi tỷ lệ lỗi — bị giới hạn tốc độ không phải là lỗi), biểu đồ thanh ngang top 8 công cụ, thanh hiệu quả subagent, phân bổ token theo mô hình, và thống kê nén. Tự làm mới mỗi 30 giây từ `/api/settings/info` và `/api/workflows`. Tooltip theo con trỏ với phát hiện cạnh viewport trên mọi biểu đồ |
...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Payload của hook được **cắt bớt trước khi lưu** vào `events.data`: bản sao toàn bộ tệp của các công cụ gốc (`originalFile` với Edit/Write, `file.content` / `file.base64` với Read) bị loại bỏ; trường cấp cao nhất `background_tasks` bị loại khỏi mọi hook và kích thước JSON tính theo byte của nó được ghi tại `data._trimmed.dropped.background_tasks`; mọi chuỗi ở bất kỳ đâu trong payload đều bị cắt còn chừng này ký tự (kèm một ghi chú ngắn về phần đã cắt). Mỗi thay đổi được ghi vào `data._trimmed` (hiển thị là **Đã cắt bớt khi lưu** trong chi tiết sự kiện). Khi bật cắt bớt, thứ được lưu là bản xem trước: khung terminal và diff của các chỉnh sửa dưới giới hạn trường vẫn hoạt động, khung "original file" không hiển thị cho sự kiện đã lưu, phần tóm tắt đếm số dòng của bản xem trước, và tìm kiếm văn bản chỉ thấy phần được giữ lại — transcript trên đĩa vẫn giữ toàn bộ văn bản. Đặt `0` để tắt toàn bộ việc cắt bớt, kể cả loại bỏ trường |
| `DASHBOARD_EVENT_FIELD_CAP` | `16384` | Ngân sách byte cho một `tool_input` / `tool_response` vẫn còn lớn sau khi cắt chuỗi (ví dụ `structuredPatch` bao trọn một tệp): vượt ngân sách thì chỉ giữ các giá trị vô hướng ngắn vừa đủ. `npm run trim-events` áp dụng cùng quy tắc cho các hàng lưu trước khi có cắt bớt hoặc khôi phục từ bản xuất cũ (mặc định chạy thử chỉ đọc; `--yes --backup` để ghi lại và VACUUM khi dashboard đã dừng) |
| `DASHBOARD_COMPACT_MANAGER_BIN` | `compact-manager` (tra PATH) | Tệp thực thi cho bảng **compact-manager readout** tùy chọn trên Dashboard (tự động nén ngữ cảnh: chế độ, ngưỡng, tình trạng watcher, mức dùng theo phiên qua `GET /api/compact-manager/status`). Bảng chỉ xuất hiện khi CLI phản hồi — máy không có compact-manager sẽ không thấy nó, không cần cấu hình |
...
| `npm run trim-events` | Thu gọn các hàng `events.data` được lưu trước khi có cắt bớt payload (mặc định chạy thử; `--yes --backup` để ghi lại và VACUUM khi dashboard đã dừng) |
