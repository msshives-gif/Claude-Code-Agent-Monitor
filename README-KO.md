...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | 훅 페이로드는 `events.data`에 **저장되기 전에 축약**됩니다: 네이티브 도구의 전체 파일 사본(Edit/Write의 `originalFile`, Read의 `file.content` / `file.base64`)은 제거되고, `tool_input` / `tool_response` 안의 모든 문자열은 이 글자 수로 잘리며, 각 변경은 `data._trimmed`에 기록됩니다(이벤트 상세에서 **저장 시 축약됨**으로 표시). 저장되는 것은 미리보기입니다: 터미널 보기와 필드 한도 이하 편집의 diff는 그대로 동작하고, "original file" 패널은 저장된 이벤트에 표시되지 않으며, 요약은 미리보기 줄 수를 세고, 텍스트 검색은 보관된 내용만 봅니다 — 전체 텍스트는 디스크의 트랜스크립트에 있습니다. `0`으로 설정하면 페이로드를 그대로 저장합니다 |
