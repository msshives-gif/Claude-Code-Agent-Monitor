...
| **대시보드**                      | `localStorage`에 저장되는 두 개의 탭: **모니터** — 개요 통계(6개 통계 카드, 비용 카드는 **API-Equiv. Cost**로 표기 — 제공사 API 정가 기준 합계(Claude 세션은 Anthropic, Codex는 OpenAI 요율표)이며 구독 실지출이 아님), 접을 수 있는 서브에이전트 계층과 **컨텍스트 사용량 게이지**(최신 요청의 컨텍스트 점유량 대 모델 윈도우, 70%/90%에서 녹색/호박색/적색)가 있는 활성 에이전트 카드, 연속된 `PreToolUse`/`PostToolUse`가 확장 가능한 “*N* tool calls · Read ×3, Bash ×2” 행으로 접히는(프롬프트, 중지, 오류, 알림은 최상위 유지) 최근 활동 피드 — `ResizeObserver`를 통해 사용 가능한 뷰포트 높이를 채우는 동적 항목 수. **상태** — 종합 시스템 상태 점수 링(가중치: 0.4 × 성공률 + 0.25 × 캐시 적중률 + 0.25 × (100 − 오류율) + 0.1 × (100 − 힙 %)), 레코드 분포를 보여주는 스토리지 엔진 도넛 차트, 캐시 성능 / 오류율 / 성공률 게이지, 도구 호출 가로 막대 차트(상위 8개), 서브에이전트 효과성 막대, 모델 토큰 분포, 컴팩션 영향 통계. 모든 상태 지표는 `/api/settings/info`와 `/api/workflows`에서 30초마다 자동 새로고침됩니다. 모든 차트에 뷰포트 가장자리 감지 기능이 있는 커서 추적 툴팁 제공 |
