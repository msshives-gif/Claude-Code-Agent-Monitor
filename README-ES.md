...
| **Panel de control**                      | Dos pestañas persistieron en `localStorage`: **Monitor** — estadísticas generales (6 tarjetas estadísticas; la tarjeta de costo se etiqueta **API-Equiv. Cost** — totales a precios de lista de la API del proveedor (tarifas de Anthropic para sesiones de Claude, de OpenAI para Codex), no gasto de suscripción), tarjetas de agentes activos con jerarquía de subagentes colapsables y un **indicador de llenado de contexto** (ocupación de contexto de la última solicitud frente a la ventana del modelo, verde/ámbar/rojo al 70%/90%), y feed de actividad reciente donde las rachas consecutivas de `PreToolUse`/`PostToolUse` se colapsan en filas expandibles “*N* tool calls · Read ×3, Bash ×2” (los prompts, stops, errores y notificaciones permanecen en el nivel superior), con conteos dinámicos de elementos que llenan la altura disponible de la ventana de visualización a través de `ResizeObserver`. **Salud** — anillo de puntuación de salud del sistema compuesto (pesado: 0,4 × tasa de éxito + 0,25 × tasa de éxito de la caché + 0,25 × (100 − tasa de error) + 0,1 × (100 − % de la pila)), gráfico de donut del motor de almacenamiento con distribución de registros, medidores de rendimiento de la caché / tasa de error / tasa de éxito más un indicador **Limits** de límites de uso (los avisos `rate_limit` se reportan ahí y se excluyen de la tasa de error — estar limitado no es un fallo), gráfico de barras horizontal de invocación de herramientas (8 principales), barras de eficacia de subagentes, distribución de tokens del modelo y estadísticas de impacto de la compactación. Todas las métricas de salud se actualizan automáticamente cada 30 s desde `/api/settings/info` y `/api/workflows`. Etiquetas de herramientas de seguimiento del cursor con detección de bordes de la ventana de visualización en todos los gráficos |
...
| `DASHBOARD_EVENT_STRING_CAP` | `2048` | Las cargas de los hooks se **recortan antes de guardarse** en `events.data`: se eliminan las copias de archivo completo de las herramientas nativas (`originalFile` en Edit/Write, `file.content` / `file.base64` en Read); se elimina el campo de nivel superior `background_tasks` de cada hook y su tamaño JSON en bytes se registra en `data._trimmed.dropped.background_tasks`; y cada cadena, esté donde esté en la carga, se corta a este número de caracteres (más una breve nota de cuánto se recortó). Cada cambio se anota en `data._trimmed` (mostrado como **Recortado al guardar** en el detalle del evento). Con el recorte activado, lo guardado es una vista previa: la vista de terminal y el diff de cualquier edición por debajo del límite de campo siguen funcionando, el panel "original file" no se muestra para eventos guardados, los resúmenes cuentan líneas de la vista previa y la búsqueda de texto solo ve lo conservado; la transcripción en disco conserva el texto completo. Ponlo en `0` para desactivar todo el recorte, incluida la eliminación de campos |
| `DASHBOARD_EVENT_FIELD_CAP` | `16384` | Presupuesto en bytes para un `tool_input` / `tool_response` que sigue siendo grande tras recortar cadenas (p. ej. un `structuredPatch` de un archivo entero): por encima solo se conservan los escalares cortos que quepan. `npm run trim-events` aplica las mismas reglas a filas guardadas antes de que existiera el recorte o restauradas de una exportación antigua (simulación de solo lectura por defecto; `--yes --backup` para reescribir y hacer VACUUM con el panel detenido) |
...
| `DASHBOARD_COMPACT_MANAGER_BIN` | `compact-manager` (búsqueda en PATH) | Ejecutable para el panel opcional de **lectura de compact-manager** en el Dashboard (auto-compactación de contexto: modo, umbrales, salud del watcher, uso por sesión vía `GET /api/compact-manager/status`). El panel solo aparece cuando el CLI responde: una máquina sin compact-manager simplemente no lo muestra, sin configuración |
...
| `npm run trim-events` | Reducir las filas de `events.data` guardadas antes de que existiera el recorte de cargas (simulación por defecto; `--yes --backup` para reescribir y hacer VACUUM con el panel detenido) |
