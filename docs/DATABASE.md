...
| `context_window` | INTEGER | YES | Context window (tokens) of the model that produced the latest usage record, resolved by `contextWindowForModel()` in `server/lib/token-usage.js` (e.g. 1M for the Claude 5 family / Opus 4.6+ / Sonnet 4.6+, 200K otherwise) |
