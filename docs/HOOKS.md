...
1. **Exact owner-process check (preferred).** The hook handler runs as a descendant of the `claude` process, so it walks its own ancestry (`findAgentAncestor`) and attaches `sender: { pid, start }` to every event payload. The walk recognizes the CLI by argv basename (`claude` directly, or `node`/`bun` launching a script whose basename is `claude`) — an install shape whose script basename differs (e.g. an npm shim resolving to `cli.js`) simply never records an owner and degrades to the cwd fallback below — `start` is a start-of-process token (`/proc/<pid>/stat` starttime on Linux, `ps -o lstart=` elsewhere) that defeats PID reuse. Ingestion persists the pair onto `sessions.owner_pid` / `owner_pid_start`, but only for **direct loopback** requests (any `X-Forwarded-For`/`Forwarded` header disqualifies — a same-host reverse proxy relays remote senders over 127.0.0.1), and the pair is cleared when the handler explicitly reports a failed lookup (`sender: {lookupFailed: true}`) or when a non-loopback event arrives for an owned session — so a resumed or migrated session never keeps a stale process identity — while a sender-less loopback API call (any client other than the hook handler) makes no claim and leaves it untouched. The reap then asks `probeAgentPidLive`: an alive owner spares the session **even when its recorded cwd matches no process** (the recorded cwd follows in-session `cd`; the process cwd stays at the launch directory — the old cwd matching falsely reaped live sessions on exactly this drift), and a verified-dead owner is authoritative even when an unrelated `claude` runs in the same directory.
2. **cwd-set fallback (legacy rows, no verdict).** When no owner is recorded (pre-upgrade rows, remote/forwarded sessions) it enumerates the matching running `claude` or `codex` CLI processes and their working directories (`ps` + `lsof` on macOS, `/proc/<pid>/cwd` on Linux) and completes an `active` local-provider session only when its matching CLI has no live process in the session's cwd.

Either way the terminal state matches what a real `SessionEnd` produces, plus a synthetic `SessionEnd` event (`data.source = "liveness-probe"`) on the timeline.
